...
func TestBufferCursorAfterMovementToTheNextByte(t *testing.T) {
	...
	buffer, err := bufferFromContent([]byte(content), []byte(nl))
	assertNoErrors(t, err)
	cursor, err := NewBufferCursor(buffer).BytesForward(1)
	assertNoErrors(t, err)
...
func TestBufferCursorAfterMovementForwardAndBackward(t *testing.T) {
	...
	assertNoErrors(t, err)
	cursor := NewBufferCursor(buffer)
	cursor, err = cursor.BytesForward(3)
	assertNoErrors(t, err)
...
func TestBufferCursorIsNewLine(t *testing.T) {
	...
	cursor, err = cursor.BytesForward(5)
	assertNoErrors(t, err)
	if !cursor.IsNewLine() {
...
func TestBufferSearchForward(t *testing.T) {
	...
	cursor, err := NewBufferCursor(buffer).SearchForward(buffer.Nl_seq())
	if !cursor.IsNewLine() {
		t.Errorf("Expected cursor to be on new line, rune: %+q", buffer.Content()[cursor.Index()])
	}
...
func TestBufferSearchBackward(t *testing.T) {
	...
	cursor, err := NewBufferCursor(buffer).ToIndex(14)
	assertNoErrors(t, err)
...
func TestBufferCursorRunesForwardOnce(t *testing.T) {
	...
	buffer, err := bufferFromContent([]byte(content), []byte(nl))
	assertNoErrors(t, err)
	cursor, err := NewBufferCursor(buffer).ToIndex(1)
	assertNoErrors(t, err)
	cursor, err = cursor.RunesForward(1)
	assertNoErrors(t, err)
...
func TestBufferCursorMultipleRunesForward(t *testing.T) {
	...
	cursor, err := NewBufferCursor(buffer).ToIndex(1)
	assertNoErrors(t, err)
	cursor, err = cursor.RunesForward(3)
	assertNoErrors(t, err)
...
func TestBufferCursorMultipleRunesBackward(t *testing.T) {
	...
	buffer, err := bufferFromContent([]byte(content), []byte(nl))
	assertNoErrors(t, err)
	cursor, err := NewBufferCursor(buffer).ToIndex(8)
	assertNoErrors(t, err)
	cursor, err = cursor.RunesBackward(3)
	assertNoErrors(t, err)
	assertIntEqualMsg(t, cursor.Index(), 3, "Expected cursor to be at the third byte: ")
	assertPointsEqual(t, cursor.BytePosition(), Point{row: 0, col: 3})
	assertPointsEqual(t, cursor.RunePosition(), Point{row: 0, col: 2})
...
func TestBufferCursorUpdateToInsertChange(t *testing.T) {
	nl := NewLineUnix
	content := "abcxyz"
	buffer, err := bufferFromContent([]byte(content), []byte(nl))
	assertNoErrors(t, err)
	cursor, err := NewBufferCursor(buffer).ToIndex(3)
	assertNoErrors(t, err)
	assertIntEqualMsg(t, cursor.Index(), 3, "Expected cursor to be at the third byte: ")
	assertPointsEqual(t, cursor.BytePosition(), Point{row: 0, col: 3})
	assertPointsEqual(t, cursor.RunePosition(), Point{row: 0, col: 3})

	buffer.Insert(1, []byte("bb"))
	cursor, err = cursor.UpdateToChange(buffer.changes[len(buffer.changes)-1])
	assertNoErrors(t, err)

	assertIntEqualMsg(t, cursor.Index(), 5, "")
	assertPointsEqual(t, cursor.BytePosition(), Point{row: 0, col: 5})
	assertPointsEqual(t, cursor.RunePosition(), Point{row: 0, col: 5})
}

func TestBufferCursorUpdateToEraseChange(t *testing.T) {
	nl := NewLineUnix
	content := "abcxyz"
	buffer, err := bufferFromContent([]byte(content), []byte(nl))
	assertNoErrors(t, err)
	...
	buffer.Erase(Region{1, 3})
	cursor, err = cursor.UpdateToChange(buffer.changes[len(buffer.changes)-1])
	assertNoErrors(t, err)

	assertIntEqualMsg(t, cursor.Index(), 1, "")
	assertPointsEqual(t, cursor.BytePosition(), Point{row: 0, col: 1})
	assertPointsEqual(t, cursor.RunePosition(), Point{row: 0, col: 1})
}

func TestBufferCursorUpdateToReplaceAdditiveChange(t *testing.T) {
	nl := NewLineUnix
	content := "abcxyz"
	buffer, err := bufferFromContent([]byte(content), []byte(nl))
	assertNoErrors(t, err)
	...
	buffer.Edit(ReplacementInput{1, 3, []byte("zyx")})
	cursor, err = cursor.UpdateToChange(buffer.changes[len(buffer.changes)-1])
	assertNoErrors(t, err)

	assertIntEqualMsg(t, cursor.Index(), 4, "")
	assertPointsEqual(t, cursor.BytePosition(), Point{row: 0, col: 4})
	assertPointsEqual(t, cursor.RunePosition(), Point{row: 0, col: 4})
}

func TestBufferCursorUpdateToReplaceRegressiveChange(t *testing.T) {
	nl := NewLineUnix
	content := "abcxyz"
	buffer, err := bufferFromContent([]byte(content), []byte(nl))
	assertNoErrors(t, err)
	...
	buffer.Edit(ReplacementInput{1, 3, []byte("z")})
	cursor, err = cursor.UpdateToChange(buffer.changes[len(buffer.changes)-1])
	assertNoErrors(t, err)

	assertIntEqualMsg(t, cursor.Index(), 2, "")
	assertPointsEqual(t, cursor.BytePosition(), Point{row: 0, col: 2})
	assertPointsEqual(t, cursor.RunePosition(), Point{row: 0, col: 2})
}

func TestBufferCursorUpdateToReplaceChangeAfterCursor(t *testing.T) {
	nl := NewLineUnix
	content := "abcxyz"
	buffer, err := bufferFromContent([]byte(content), []byte(nl))
	assertNoErrors(t, err)
	...
	buffer.Edit(ReplacementInput{4, 6, []byte("z")})
	cursor, err = cursor.UpdateToChange(buffer.changes[len(buffer.changes)-1])
	assertNoErrors(t, err)

	assertIntEqualMsg(t, cursor.Index(), 3, "")
	assertPointsEqual(t, cursor.BytePosition(), Point{row: 0, col: 3})
	assertPointsEqual(t, cursor.RunePosition(), Point{row: 0, col: 3})
}
